...
BEGIN
  ...
  WHERE 
    ...
    appointment_date BETWEEN (start_time AT TIME ZONE 'UTC')::DATE AND (end_time AT TIME ZONE 'UTC')::DATE
  LIMIT 1;

  IF FOUND THEN
    -- Determinar si es un bloqueo completo o parcial
    IF (blocked_time.start_time AT TIME ZONE 'UTC')::DATE < appointment_date AND 
       (blocked_time.end_time AT TIME ZONE 'UTC')::DATE > appointment_date THEN
      -- Día completamente bloqueado
      RAISE EXCEPTION 'El día % está completamente bloqueado: %',
        TO_CHAR(appointment_date, 'DD/MM/YYYY'),
        blocked_time.reason;
    ELSIF (blocked_time.start_time AT TIME ZONE 'UTC')::DATE = appointment_date AND 
          (blocked_time.end_time AT TIME ZONE 'UTC')::DATE = appointment_date THEN
      ...
      IF (NEW.date, new_end_time) OVERLAPS (blocked_time.start_time, blocked_time.end_time) THEN
        RAISE EXCEPTION 'Horario bloqueado de % a %: %',
          TO_CHAR(blocked_time.start_time AT TIME ZONE 'UTC', 'HH24:MI'),
          TO_CHAR(blocked_time.end_time AT TIME ZONE 'UTC', 'HH24:MI'),
          blocked_time.reason;
      END IF;
    ELSIF (blocked_time.start_time AT TIME ZONE 'UTC')::DATE = appointment_date THEN
      ...
      IF NEW.date >= blocked_time.start_time THEN
        RAISE EXCEPTION 'Horario bloqueado desde las %: %',
          ...
          blocked_time.reason;
      END IF;
    ELSIF (blocked_time.end_time AT TIME ZONE 'UTC')::DATE = appointment_date THEN
  ...
  WITH all_appointments AS (
    ...
    AND (a.date AT TIME ZONE 'UTC')::DATE = appointment_date
    AND a.id != NEW.id
    UNION ALL
    SELECT 
      a.date as start_time,
